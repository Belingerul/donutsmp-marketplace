      ...
      <div className="mx-auto max-w-md px-4 py-12">
        <div className="rounded-[28px] border border-zinc-800 bg-zinc-950/60 p-6">
          ...
          <div className="mt-6 grid gap-3">
            ...
            <button onClick={signup} disabled={loading} className="h-14 rounded-2xl bg-gradient-to-r from-fuchsia-600 to-cyan-500 px-6 text-base font-semibold disabled:opacity-60">
              {loading ? "…" : "Create"}
            </button>
            {err ? <div className="text-sm text-red-300">{err}</div> : null}
            <a className="text-sm text-zinc-400 underline" href="/signin">I already have an account</a>
          </div>
        </div>
      </div>
    </main>
